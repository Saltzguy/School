...
filename = '.\Part2\data\goldenSouth.txt'

def clean_up(filename=filename):
...
with tf.Session() as sess:
    writer = tf.summary.FileWriter('./graphs/Lab3', sess.graph)
    sess.run(init)
    average_loss = 0
    for step in range(num_steps):
        ...
        _, loss_val, summary = sess.run([trainer, loss, merged_summary_op], feed_dict=feed_dict)
